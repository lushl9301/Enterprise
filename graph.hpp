...
::graph
	...
	if (file != NULL) {
		...
		ret = fread(
			tmp_beg_pos, sizeof(file_index_t),
			vert_count + 1, file);
	...
	if (file != NULL) {
		...
		if (posix_memalign((void **) &tmp_weight, 32, sizeof(file_weight_t) * edge_count))
			perror("posix_memalign");
		...
		if (sizeof(file_weight_t) != sizeof(new_weight_t)) {
			if (posix_memalign((void **) &weight, 32, sizeof(new_weight_t) * edge_count))
				perror("posix_memalign");
			for (new_index_t i = 0; i < edge_count; ++i)
