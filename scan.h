...
__global__ void __post_scan(
	...
	while (tid < num_dat) {
		scan_out_d[tid] = scan_out_d[tid] + blk_sum[blk_idx] + s_mem[grd_idx];
		if (tid + 1 < num_dat)
			scan_out_d[tid + 1] = scan_out_d[tid + 1] + blk_sum[blk_idx] + s_mem[grd_idx];
		...
		if (!((blk_idx - blockIdx.x) % (gridDim.x << 1))) {
			grd_idx_orig++;
			grd_idx = grd_idx_orig + CONFLICT_FREE_OFFSET(grd_idx_orig);
		}
	}
}
...
__global__ void __post_scan(
	...
	index_t blk_idx = blk_idx_orig + CONFLICT_FREE_OFFSET(blk_idx_orig);
...
__global__ void __insp_post_scan(
	...
	while (tid < num_dat) {
		scan_out_d[tid] = scan_out_d[tid] + s_mem[blk_idx];
		scan_out_d[tid + 1] = scan_out_d[tid + 1] + s_mem[blk_idx];
		tid += P_DATA;
		blk_idx_orig += gridDim.x;
		blk_idx = blk_idx_orig + CONFLICT_FREE_OFFSET(blk_idx_orig);
	}
...
__host__ void lrg_scan(
	...
	cudaMalloc((void **) &blk_sum, sizeof(data_t) * num_blk);

	cudaMalloc((void **) &grd_sum, sizeof(data_t) * num_grd);
	if (num_grd > (THD_NUM << 1))
		std::cout << "Out of Range\n";
	__pre_scan<data_t, index_t><<< BLK_NUM, THD_NUM, (padding + (THD_NUM << 1)) * sz, stream >>>
		(
			scan_ind_d,
				adj_card_d,
				scan_out_d,
	...
	__spine_scan<data_t, index_t><<< BLK_NUM, THD_NUM, (padding + (THD_NUM << 1)) * sz, stream >>>
		(
			blk_sum,
				grd_sum,
				num_blk,
	...
	cudaThreadSynchronize();
	__post_scan<data_t, index_t><<< BLK_NUM, THD_NUM, (padding + (THD_NUM << 1)) * sz, stream >>>
		(
			scan_out_d,
				blk_sum,
				grd_sum,
